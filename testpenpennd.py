...
def todownload(t):
    mkdir('./' + Year + month + day + ' ' + ID + 'ts_files/')
    url = 'https://video-ws-ak-hls.lv-play.com/live/' + ID + 'Y/' + t
    r = requests.get(url)
    if r.status_code == 200:
        tspath = './' + Year + month + day + ' ' + ID + 'ts_files/' + t.split('.')[0] + '.ts'
        ...
        with open(ID + 'failQQ.txt', 'a') as f:
...
def failfiledownload(ID):
    mkdir('./' + ID +'ts_files/')
    with open(ID + 'failQQ.txt', 'r') as f:
        ts_list = f.readlines()
        # print(ts_list)
    ...
    for i in ts_list:
        ...
        while dlcount < 20:
            r = requests.get(url)
            if r.status_code == 200:
                tspath = './' + ID +'ts_files/' + i.split('.')[0].replace('https://video-ws-ak-hls.lv-play.com/live/' + ID + 'Y/', '') +'.ts'
                print('正在下載' + i.split('.')[0].replace('https://video-ws-ak-hls.lv-play.com/live/' + ID + 'Y/', '') + '.ts')
                with open(tspath, "wb+") as file:
                    for chunk in r.iter_content(chunk_size=1024):
                        if chunk:
                            file.write(chunk)
...
def revdownload(n, ID):
    ...
    while True:
        ...
        if r.status_code == 200:
            mkdir('./' + Year + month + day + ' ' + ID + 'ts_files/')
            tspath = './' + Year + month + day + ' ' + ID + 'ts_files/' + str(n) + '.ts'
            with open(ID + 'failQQ.txt', 'a') as f:
...
def marge(ID):
    file_list = []
    for root, dirs, files in os.walk('./' + Year + month + day + ' ' + ID +'ts_files/'): # 生成器
        # dirs.sort()
        for fn in files:
            p = str(root+'/'+fn)
            file_list.append(p)
    ...
    # for p in file_list:
        # print(p)
    mkdir('./合併檔案')
    file_path = './合併檔案/' + Year + month + day + ' ' + ID + '.ts'
